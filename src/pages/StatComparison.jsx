...
import {Container,Grid,MenuItem,Select,FormControl,InputLabel,Button,Card,CardContent,Typography,Box,Paper,Checkbox,ListItemText,} from "@mui/material";
import {SimpleBarChart,SimpleLineChart,SimplePieChart,MultiBarChart,MultiHorizontalBarChart,BubbleChart,CirclePackingChart,} from "./../components/statistics_components/Charts";
...
export default function Dashboard() {
  ...
  const availableYears = years.filter((y) => {
    ...
    if (selectedDept && !selectedStat)
      return mockData[y]?.[selectedDept] !== undefined;
    if (selectedDept && selectedStat)
      return mockData[y]?.[selectedDept]?.[selectedStat] !== undefined;
    return false;
  });
  ...
  const handleShowData = () => {
    if (selectedYear && selectedDept && selectedStat) {
      ...
      if (value) {
        setCards([
          { year: selectedYear, dept: selectedDept, stat: selectedStat, value },
        ]);
    ...
    selected.forEach((year) => {
      if (!cards.some((c) => c.year === year)) {
        ...
        if (value) {
          setCards((prev) => [
            ...prev,
            { year, dept: displayDept, stat: displayStat, value },
          ]);
        }
      }
    });
  ...
  const combinedChart = () => {
    ...
    if (type === "bubble") {
      if (cards.length === 1) {
        ...
        return (
          <Box
            sx={{width: "100%",mt: 2,display: "flex",justifyContent: "center",}}
          >
            <Box
              sx={{width: "100%",maxWidth: 900,height: "70vh",minHeight: 300,}}
            >
    ...
    PaperProps: {
      sx: {
        "& .MuiMenuItem-root": {
          ...
          "&:hover": {
            backgroundColor: "#e0e0e0", // hover grey
          },
        },
      },
    },
